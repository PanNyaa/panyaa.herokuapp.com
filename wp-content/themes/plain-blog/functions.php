...
function my_tag_cloud_args($args) {
    $myargs = array(
        'smallest' => 8, // 最小文字サイズ
        'largest' => 16, // 最大文字サイズ
        'number' => 0,  // 一度に表示するのは全てのタグ
      //'order' => 'RAND', // 表示順はランダムで
    );
    $args = wp_parse_args( $args, $myargs );
    return $args; //myargs
}
add_filter('widget_tag_cloud_args', 'my_tag_cloud_args'); //my_tag_cloud_filter
...
function plain_blog_loadStyles() {
    ...
    //wp_enqueue_style('plain-blog-style', get_template_directory_uri() . '/style.css/' ,false,false);
